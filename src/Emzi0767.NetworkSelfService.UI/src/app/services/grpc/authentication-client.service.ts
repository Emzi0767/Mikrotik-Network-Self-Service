...
const EXTRACT_SESSION = makeExtractor<AuthenticationResponse.AuthenticationSuccess>(new GrpcMessagePool([ Empty, AuthenticationResponse.AuthenticationSuccess ]));
...
export class AuthenticationClientService {
  ...
  authenticate(username: string, password: string, remember: boolean): Observable<AuthenticationResponse.AuthenticationSuccess> {
  ...
  refreshSession(refreshToken: string): Observable<AuthenticationResponse.AuthenticationSuccess> {
